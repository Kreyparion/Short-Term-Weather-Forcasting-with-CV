...
def custom_ms2e(y_actual,y_pred):                # fonction d'erreur qui sert dans les modèles neuronaux => on l'applique sur des tenseurs
    custom_loss=kb.square(kb.square(4*(y_actual-y_pred)))
    return custom_loss

def custom_mse(y_actual,y_pred):                # fonction d'erreur qui sert dans les modèles neuronaux => on l'applique sur des tenseurs
    custom_loss=kb.square(255*(y_actual-y_pred))
    return custom_loss
...
#########
#########
nbjours = 80 # on peut aller jusqu'à 124  (nombre de jours en banque de données à partir du 30 mai)
...
t = tableau_images("./Images/Total/VIS8_MSG4-SEVI-MSG15-0100-NA-2019", "jpg", nbjours, 5, 30, (9, 42), (17, 28), (750,750),
...
new_model.compile(optimizer='rmsprop', loss=custom_mse, metrics=['mae', 'mse'])
...
new_model.save_weights('./Model-Res/Weights/' + Temps + "/" + Temps + '-c2m1')
    # sauvegarde du modèle entier
new_model.save('./Model-Res/' + Temps + '-c2m1.h5')
