...
#########
#########
nbjours = 7  # on peut aller jusqu'à 52 jours  (nombre de jours en banque de données du 9 août au 30 septembre)
...
#rnn_layer1 = tf.keras.layers.ConvLSTM2D(16, (1,1),  return_sequences=True, stateful=True, data_format = "channels_last")(pool_layer3)
#drop1 = tf.keras.layers.Dropout(0.2) (rnn_layer1)
#batch1 = tf.keras.layers.BatchNormalization(name = "batch1")(drop1)
#rnn_layer2 = tf.keras.layers.ConvLSTM2D(32, (1,1),  return_sequences=True, stateful=True, data_format = "channels_last")(batch1)
#rnn_layer2 = batch1
...
'''
#bloc déconv 1
upool_layer1 = tf.keras.layers.Conv3DTranspose(nbfiltreini, (1, 4, 4), strides=(1, 2, 2), activation='relu', data_format="channels_last")(pool_layer1) # 108 -> 218
...
single_step_history = single_step_model.fit(x_train[:train_dim], y_train[:train_dim], epochs=3, validation_data= (x_train[train_dim:-nbpredict], y_train[train_dim:-nbpredict]), validation_steps=10)
