...
#########
#########
nbjours = 8  # on peut aller jusqu'à 124  (nombre de jours en banque de données à partir du 30 mai)
...
upool_layer1 = tf.keras.layers.Conv3DTranspose(4, (1, 4, 4), strides=(1, 2, 2), activation='relu', data_format="channels_last")(normap1)
print('upool_layer1 : ' + str(upool_layer1.shape))
normau1 = tf.keras.layers.BatchNormalization()(upool_layer1)
#upool_layerconv1 = tf.keras.layers.Conv3D(4, (1, 3, 3), strides=(1, 1, 1),  activation='relu',data_format="channels_last")(normau1)
#print('upool_layerconv1 : ' + str(upool_layerconv1.shape))
#normauc1 = tf.keras.layers.BatchNormalization()(upool_layerconv1)
#deconv_layer2 = tf.keras.layers.Conv3DTranspose(4, (1, 3, 3), strides=(1, 1, 1), data_format="channels_last")(normauc1)
...
#new_model.compile(optimizer='rmsprop', loss='mse' , metrics=['mae', 'mse', 'msle', 'acc', 'mape'])
...
ecart = 0.5
loss1 = 1
single_step_model.fit(x_train[:train_dim], y_train[:train_dim], epochs=1, )
...
for i in range(0, 5):   # on fait entre 1 et 5 époques supplémentaires
    single_step_model.fit(x_train[:train_dim], y_train[:train_dim], epochs=1, )
    ev = single_step_model.evaluate(x_train[train_dim + 1:train_dim + 2], y_train[train_dim + 1:train_dim + 2],
                                    batch_size=batchsize)
...
plt.subplot(2, 1, 2)
plt.plot(new_model.history['loss'], label='Training Loss (MSE)')
